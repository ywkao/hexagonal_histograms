...
if __name__ == "__main__":
    ...
    with open('./test_01.txt', 'w') as fout:
        ...
        for line in contents[1:223]:
            ...
            #fout.write("{0} {1}\n".format(globalchannelId, sicell))
            #print("{0} {1} {2}").format(globalchannelId, hgcrocPin, sicell)
            #print "%3d %d %d %2d %2d %2d" % (globalchannelId, roc, halfroc, seq, sicell, t)

            chType = 0 if isinstance(pin,int) else 1
            final[sicell] = (roc, 39*halfroc + seq, chType)

        for pad, info in final.items():
            asic,ch,chType = info
            fout.write("{0},{1},{2},{3}\n".format(pad,asic,ch,chType))
    ...
    with open('./test_02.txt', 'w') as fout:
        fout.write("# globalchannelId padId\n")
        final = {} 
        for line in contents[1:199]:
            pad, asic, ch, chType = tuple([int(ele) for ele in line.strip().split(',')])
            if chType==1: continue # calibration channel
            globalchannelId = 78*asic + ch
            final[globalchannelId] = pad

        # sort globalchannelId
        for globalchannelId, pad in final.items():
            fout.write("{0} {1}\n".format(globalchannelId, pad))
